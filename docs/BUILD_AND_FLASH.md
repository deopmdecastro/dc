...
| Mesmo com o `sdkconfig.defaults` já aplicado, `stack overflow in task main` aparece logo a seguir aos logs de Wi-Fi (`wifi:lmac stop hw txq`, `EspWifi dropped`, `NvsDefault dropped`) | `network::spawn_network_task` não criava mesmo uma task/thread — o bring-up (e o teardown, por devolver logo a seguir ao `start()`) do `EspWifi` corria de forma síncrona dentro da própria `main`, empilhado por cima do Display+Slint, e estourava os 32 KB | Corrigido: `spawn_network_task` agora usa `std::thread::Builder::new().stack_size(...)` para correr numa thread própria com stack dedicada (ver `firmware/src/network.rs`) |
| `error[E0521]: borrowed data escapes outside of function` a apontar para `modem` / `Modem<'1>` ao compilar | `main.rs` duplicava o `Peripherals` inteiro com `unsafe { core::ptr::read(&peripherals as *const _) }` só para poder passar uma cópia ao `Display::init` e ainda ficar com `peripherals.modem` a seguir — isso corrompia o lifetime do `modem`, que deixava de ser `'static` | Corrigido: `Display::init` passou a receber só o `SPI2` e o `TIMER0`/`CHANNEL0` do LEDC (não o `Peripherals` todo), pelo que `peripherals.modem` fica livre com lifetime normal, sem qualquer `unsafe` — ver `firmware/src/display.rs` e `firmware/src/main.rs` |
