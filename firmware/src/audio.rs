...
use crate::pinout::pins::{AMP_EN, I2S_BCLK, I2S_DIN, I2S_DOUT, I2S_WS};
...
static AUDIO_VOLUME: AtomicU8 = AtomicU8::new(75);
...
pub fn set_volume(level: u8) {
    AUDIO_VOLUME.store(level.min(100), Ordering::Relaxed);
}
...
unsafe fn init_i2s_tx() -> Result<()> {
    ...
    if !I2S_INITIALIZED.load(Ordering::SeqCst) {
        ...
        cfg.mode =
            (esp_idf_sys::i2s_mode_t_I2S_MODE_MASTER | esp_idf_sys::i2s_mode_t_I2S_MODE_TX) as u32;
        ...
        let pins = esp_idf_sys::i2s_pin_config_t {
            bck_io_num: I2S_BCLK as i32,
            ws_io_num: I2S_WS as i32,
            data_out_num: I2S_DOUT as i32,
            data_in_num: -1,
            mck_io_num: -1,
        };
        let err = esp_idf_sys::i2s_driver_install(port, &cfg, 0, core::ptr::null_mut());
        if err != 0 && err != esp_idf_sys::ESP_ERR_INVALID_STATE {
            return Err(anyhow!("i2s_driver_install TX: {err}"));
        }
        let err = esp_idf_sys::i2s_set_pin(port, &pins);
        if err != 0 {
            return Err(anyhow!("i2s_set_pin TX: {err}"));
        }
        I2S_INITIALIZED.store(true, Ordering::SeqCst);
        I2S_TX_MODE.store(true, Ordering::SeqCst);
        log::info!(
            "I2S TX inicializado: BCLK={}, WS={}, DOUT={}",
            I2S_BCLK,
            I2S_WS,
            I2S_DOUT
        );
...
unsafe fn init_i2s_rx() -> Result<()> {
    ...
    if !I2S_INITIALIZED.load(Ordering::SeqCst) {
        let mut cfg: esp_idf_sys::i2s_config_t = core::mem::zeroed();
        cfg.mode =
            (esp_idf_sys::i2s_mode_t_I2S_MODE_MASTER | esp_idf_sys::i2s_mode_t_I2S_MODE_RX) as u32;
        cfg.sample_rate = SAMPLE_RATE;
        cfg.bits_per_sample = esp_idf_sys::i2s_bits_per_sample_t_I2S_BITS_PER_SAMPLE_16BIT;
        cfg.channel_format = esp_idf_sys::i2s_channel_fmt_t_I2S_CHANNEL_FMT_ONLY_LEFT;
        cfg.communication_format = esp_idf_sys::i2s_comm_format_t_I2S_COMM_FORMAT_STAND_I2S;
        ...
        let pins = esp_idf_sys::i2s_pin_config_t {
            ...
            ws_io_num: I2S_WS as i32,
            data_out_num: -1,
            data_in_num: I2S_DIN as i32,
            mck_io_num: -1,
        };
        let err = esp_idf_sys::i2s_driver_install(port, &cfg, 0, core::ptr::null_mut());
        if err != 0 && err != esp_idf_sys::ESP_ERR_INVALID_STATE {
            return Err(anyhow!("i2s_driver_install RX: {err}"));
        }
        let err = esp_idf_sys::i2s_set_pin(port, &pins);
        if err != 0 {
            return Err(anyhow!("i2s_set_pin RX: {err}"));
        }
        I2S_INITIALIZED.store(true, Ordering::SeqCst);
...
fn write_tone(tone: u8) -> Result<()> {
    unsafe {
        ...
        let gain = volume_gain();
        let mut pcm = vec![0i16; samples];
        for (i, sample) in pcm.iter_mut().enumerate() {
            let phase =
                (i as f32 * freq as f32 * core::f32::consts::TAU / SAMPLE_RATE as f32).sin();
            *sample = (phase * 7000.0 * gain) as i16;
        }
        let mut written = 0usize;
        let err = esp_idf_sys::i2s_write(
            port,
        ...
        if err != 0 {
            return Err(anyhow!("i2s_write: {err}"));
        }
        let _ = esp_idf_sys::i2s_zero_dma_buffer(port);
        log::info!(
            "Audio I2S: tom de teste emitido em GPIO {} ({} bytes)",
            I2S_DOUT,
            written
        );
    }
...
{
    log::info!(
        "Audio I2S pronto ({} Hz, BCLK={}, WS={}, DIN={}, DOUT={})",
        SAMPLE_RATE,
        I2S_BCLK,
        I2S_WS,
        I2S_DIN,
...
fn bytes_to_pcm(data: &[u8]) -> Vec<i16> {
    ...
    for chunk in data.chunks_exact(2) {
        ...
        samples.push((sample as f32 * gain) as i16);
    }
    samples
}

fn volume_gain() -> f32 {
    AUDIO_VOLUME.load(Ordering::Relaxed) as f32 / 100.0
}
